...
firmware[19] = 0b00001010000000010100000001000000 
               #H <- MDR; 
firmware[20] = 0b00000000000001011000000100000000  
               #X <- H << 1; GOTO MAIN;
               
#X = mem[address] >> 1  
firmware[21] = 0b00001011000000110101001000001001  
               #PC <- PC + 1; fetch; 
firmware[22] = 0b00001011100000010100100000010010  
               #MAR <- MBR; read; 
firmware[23] = 0b00001100000000010100000001000000 
               #H <- MDR; 
firmware[24] = 0b00000000000010011000000100000000  
               #X <- H >> 1; GOTO MAIN;
               
#X = X AND mem[address] 
firmware[25] = 0b00001101000000110101001000001001  
               #PC <- PC + 1; fetch; 
firmware[26] = 0b00001101100000010100100000010010  
               #MAR <- MBR; read; 
firmware[27] = 0b00001110000000010100000001000000 
               #H <- MDR; 
firmware[28] = 0b00000000000000001100000100000011  
               #X <- X AND H; GOTO MAIN;
...
firmware[31] = 0b00010000000000010100000001000000
               #H <- MDR;
firmware[32] = 0b00000000000000110011000100000011
               #X <- X % H; GOTO MAIN;
